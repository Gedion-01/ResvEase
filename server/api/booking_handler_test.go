...
func TestUserGetBooking(t *testing.T) {
	...
	var (
		nonAuthUser    = fixtures.AddUser(db.Store, "John", "jim", false)
		user           = fixtures.AddUser(db.Store, "james", "foo", false)
		hotel          = fixtures.AddHotel(db.Store, "New Hotel", "Uk", 4, nil)
		room           = fixtures.AddRoom(db.Store, "small", true, 200.5, hotel.ID)
		from           = time.Now()
		till           = from.AddDate(0, 0, 6)
...
func TestAdminGetBookings(t *testing.T) {
	...
	// test non admins can't access the bookings
	req = httptest.NewRequest("GET", "/", nil)
	req.Header.Add("X-Api-Token", CreateTokenFromUser(user))
	resp, err = app.Test(req)
	if err != nil {
